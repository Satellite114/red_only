...
__ALIGN_END =
{
  /* USER CODE BEGIN USBD_CustomHID_ReportDesc */
        ...
        0x09, 0xFF,       // USAGE (Vendor Usage 1)
...
uint8_t *USBD_Get_String_Framework(ULONG *Length)
{
  ...
  /* Update final count to include the last string */
  count += len + 1;
  ...
  /* Note: Cannot use strlen() because string_framework contains binary data with null bytes */
  *Length = count;
...
uint8_t *USBD_Get_Language_Id_Framework(ULONG *Length)
{
  ...
  /* Note: Cannot use strlen() because language_id_framework contains binary data */
  *Length = count;
