...
/**
  * @brief  Complete Flash test function
  * @note   Test erase, write, read and data verification functions
  *
  * @param  *Handle: Pointer to SPIF_HandleTypeDef structure
  *
  * @retval bool: true or false
  */
bool SPIF_Test(SPIF_HandleTypeDef *Handle)
{
  uint8_t write_buffer[256];
  uint8_t read_buffer[256];
  bool test_result = true;
  uint32_t test_sector = 0;  // Test sector 0
  uint32_t test_address = 0x0000;  // Test address
  
  printf("\r\n========== SPI Flash Test Start ==========\r\n");
  
  // 1. Display Flash information
  printf("\r\n[1] Flash Information:\r\n");
  ...
  printf("    Test data prepared (0x00-0xFF)\r\n");
  
  // 3. Erase sector test
  printf("\r\n[3] Erasing sector %lu...\r\n", test_sector);
  if (SPIF_EraseSector(Handle, test_sector))
  {
  ...
  if (SPIF_WriteAddress(Handle, test_address, write_buffer, 256))
  {
    printf("    Data write successful! (256 bytes)\r\n");
  }
  else
  {
  ...
  if (SPIF_ReadAddress(Handle, test_address, read_buffer, 256))
  {
    printf("    Data read successful! (256 bytes)\r\n");
  }
  else
  {
  ...
  for (int i = 0; i < 256; i++)
  {
    ...
    {
      printf("    Verification failed! Address[%d]: Written=0x%02X, Read=0x%02X\r\n", 
  ...
  if (verify_ok)
  {
    printf("    Data verification successful! All data matched!\r\n");
  }
  
  // 7. Display first 16 bytes of data as sample
