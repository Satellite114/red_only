##########################################################################################################################
# File automatically-generated by tool: [projectgenerator] version: [4.7.0-B52] date: [Mon Dec 15 17:38:36 CST 2025] 
##########################################################################################################################
...
# ASM sources with preprocessing (.S)
ASMM_SOURCES = \
...
# macros for gcc
# AS defines
AS_DEFS = -DTX_SINGLE_MODE_NON_SECURE=1
-DTX_SINGLE_MODE_NON_SECURE=1

# C defines
C_DEFS = -DUSE_HAL_DRIVER -DSTM32U575xx -DUX_INCLUDE_USER_DEFINE_FILE -DTX_INCLUDE_USER_DEFINE_FILE -DTX_SINGLE_MODE_NON_SECURE=1

# AS includes
AS_INCLUDES =

# C includes (flattened to avoid line-break parsing issues on Windows shells)
C_INCLUDES = -ICore/Inc -IAZURE_RTOS/App -IUSBX/App -IUSBX/Target -IDrivers/STM32U5xx_HAL_Driver/Inc -IDrivers/STM32U5xx_HAL_Driver/Inc/Legacy -IMiddlewares/ST/threadx/common/inc -IDrivers/CMSIS/Device/ST/STM32U5xx/Include -IMiddlewares/ST/threadx/ports/cortex_m33/gnu/inc -IMiddlewares/ST/usbx/common/core/inc -IMiddlewares/ST/usbx/ports/generic/inc -IMiddlewares/ST/usbx/common/usbx_stm32_device_controllers -IMiddlewares/ST/usbx/common/usbx_device_classes/inc -IDrivers/CMSIS/Include
...
CFLAGS += -ILCD -ILora -IFLASH -Ilib -ITINYC256_192
...
# Explicit rules to help Windows make find .S sources in subdirectories
$(BUILD_DIR)/%.o: Core/Src/%.S Makefile | $(BUILD_DIR)
	$(AS) -c $(CFLAGS) $< -o $@

$(BUILD_DIR)/%.o: Middlewares/ST/threadx/ports/cortex_m33/gnu/src/%.S Makefile | $(BUILD_DIR)
	$(AS) -c $(CFLAGS) $< -o $@
