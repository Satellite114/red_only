...
ir_error_t short_cmd_read(vdcmd_short_header_t *ptVdCmdShortHeader, uint8_t byLen, uint8_t *pbyData)
{
    ...
    while (byLen)
    {
        ...
        rst = vdcmd_read_hanle.handle(vdcmd_read_hanle.slave_id, I2C_VD_BUFFER_RW + sizeof(vdcmd_short_header_t), wTmpLen, pbyData);
        if (rst != IR_SUCCESS)
        {
...
ir_error_t standard_cmd_read(vdcmd_std_header_t *ptVdCmdHeader, uint16_t wLen, uint8_t *pbyData)
{
    ...
    while (wLen)
    {
        ...
        rst = vdcmd_read_hanle.handle(vdcmd_read_hanle.slave_id, I2C_VD_BUFFER_RW + sizeof(vdcmd_std_header_t), wTmpLen, pbyData);
        if (rst != IR_SUCCESS)
        {
...
ir_error_t standard_cmd_write(vdcmd_std_header_t *ptVdCmdHeader, uint16_t wLen, uint8_t *pbyData)
{
    ...
    dwAddr = ((uint32_t)ptVdCmdHeader->byPara << 24) + ((uint32_t)ptVdCmdHeader->byAddr_h << 16) + ((uint32_t)ptVdCmdHeader->byAddr_l << 8) + ptVdCmdHeader->byAddr_ll;
    do
    {
